...
contract Proxy {
    ...
    function Proxy(address _logicContract) public {
        logicContract = _logicContract;
    }
    
    function() public payable {
        address _logicContract = logicContract;
        require(_logicContract != address(0));

        assembly {
            // 将 calldata 复制到内存
            calldatacopy(0, 0, calldatasize)
            // 调用逻辑合约
            let result := delegatecall(gas, _logicContract, 0, calldatasize, 0, 0)
            // 处理返回值
